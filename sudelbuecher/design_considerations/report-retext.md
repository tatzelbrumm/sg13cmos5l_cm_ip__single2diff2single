...
> **Rendering note.** Display equations use fenced ` ```math ` blocks,
> which GitHub and GitLab both render. Inline expressions are plain-text
> code spans, since GitHub writes inline math as `$x$` while GitLab writes
> it as ``$`x`$``. A ReText/MathJax variant with real inline math is in
> `report-retext.md`.
...
and SR all scale `∝ I` while noise power scales `∝ 1/I`. The
...
|---|---|---|
| `g_m/I_D` | V⁻¹ | Efficiency coordinate; everything else is downstream of where you sit on this curve |
| `(g_m/I_D)·f_T` | Hz/V | The honest speed–efficiency invariant; peaks in moderate inversion, `IC ≈ 0.1…1` |
| `g_m r_o` | — | Intrinsic gain; sets static gain error `1/(β A_0)`. In 130 nm: `≈ 25…30` at `L_min`, `100…300` at `L ≈ 1` µm |
| `I_out,peak/I_Q` | — | Class-AB current gain. Class A pins this at 1. Decent AB stages: 10–100 |
| `V_out,pp/V_DD` | — | Swing efficiency, i.e. `Σ V_DSsat` lost |
| `A_VT = V_os√(WL)` | mV·µm | Matching constant, `≈ 3…5` mV·µm thin-oxide; useful normalized form is `V_os/V_out,pp` |
| CMRR, PSRR | dB | At the frequency of interest |
| GBW`/f_T` | — | Technology utilization; usually humiliating, and usefully so |
| `SR/(GBW·V_step)` | — | `>2π` means settling is linear rather than slew-limited |
...
- NEF / PEF, or the raw invariant `v_n,in²·I_DD`
- HD3 or SFDR, quoted at fixed `V_out,pp/V_DD` **and** fixed
  `g_m/I_D` — otherwise it measures the overdrive choice, not the topology

### 2.3 Pad-specific, and where such buffers actually fail

- **Transparency**: `C_in/C_tapped node`, plus kickback
...
**Area.** Offset and matching drag it in through `A_VT/√(WL)`, so any
...
1. swing efficiency `V_out,pp/Σ V_DSsat` on the 3.3 V
   thick-oxide rail;
...
| 6 | *Low-voltage class-AB output stages using floating capacitors* — the sub-`2V_GS` alternative to Monticelli | Electron. Lett. lineage | [PDF](https://www.academia.edu/22725840/Low_voltage_class_AB_output_stages_for_CMOS_op_amps_using_floating_capacitors) |
...
than `2V_GS` of headroom but needs no capacitor, whereas the
...
with tail-current steering so `g_m,tot` stays flat as the common
...
**Headroom.** Monticelli's mesh needs roughly `2V_GS + 2V_DSsat` of
...
one threshold, where Monticelli's requires more than `2V_GS`. On a 3.3 V
thick-oxide rail with `V_T ≈ 0.5…0.7` V this is affordable; on the
...
The floating class-AB control `M_19/M_20` shifted *into* the folded-cascode
summing circuit means the AB control contributes neither noise nor offset: the
two bias sources `I_b6/I_b7` that would otherwise sit in parallel with the
cascodes `M_14/M_16` are eliminated outright. The floating current source
`M_27`–`M_28`, built with the *same* translinear structure as the AB
control so that its supply dependence cancels the control's, yields an `I_Q`
that does not walk with `V_DD`. A real solution to a real problem, at
a cost of two transistors.

Table I of that paper was measured at exactly 3.3 V — the same rail as IOVDD
on the 5L shuttle — with two stacked `V_GS` in the output stage, which on
IHP thick-oxide devices (`V_T ≈ 0.5…0.7` V) leaves room.
...
switches, the 3× mirrors `M_6`–`M_7` and `M_9`–`M_10`, plus `M_29`–`M_31`
...
### 6.3 The porting obstacle — `C_M1/C_M2`
...
- **Gain.** Their 85 dB came from 1 µm devices. On 130 nm thick-oxide with the
  ...
  Geelen gain boosting on `M_14/M_16` becomes less optional.
- **Load.** 10 kΩ ‖ 10 pF at 3 mA peak is a *chip-level* buffer. A metre of
  ...
  for that raises `C_GS,out`, drags the output pole down, and forces
  `C_M` up — straight back into the capacitor problem. Size that loop before
  committing.

Their figure of merit, `F = B/P_sup` at `C_L = 10` pF, is
supply-referred rather than current-referred; convert to
`GBW·C_L/I_DD` to compare against anything modern.
...
10–15 pF. **Budget `C_L ≈ 15` pF** as the honest bench number, and
50–100 pF if anyone ever hangs un-terminated coax on it.

At `C_L = 15` pF:

| Target GBW | Required `g_m` | `I_D` at `g_m/I_D = 10` V⁻¹ |
|---|---|---|
| 10 MHz | 0.94 mS | ≈ 95 µA |
| 100 MHz | 9.4 mS | ≈ 1 mA |

Current is not the wall. **The non-dominant poles are.** A thick-oxide 3.3 V
device at `L ≈ 1` µm in moderate inversion has `f_T` of perhaps
...
Full-power bandwidth requires `SR = 2π f V_pk`, i.e. peak
current `I = C_L·SR`. For 1 V peak at 10 MHz into 15 pF that is
...
- Noise scales as `√(BW)`. If the path characterizes anything
...
The derivation is one line. For `V_out = V_pksin(2π f t)`,
...
```math
\mathrm{FPBW} = \frac{\mathrm{SR}}{2\pi V_\mathrm{pk}}
```
...
A class-A stage with fixed tail current `I` has `SR = I/C_L`, and its
...
The ratio `SR/(GBW·V_step)` from §2.1 is the
same idea in dimensionless form. Above about `2π`, the amplifier settles by
...
1 V peak at 10 MHz needs `SR = 2π×10^7×1 ≈ 63` V/µs;
into 15 pF, `I = C·SR ≈ 0.94` mA — the same figure as §7.1.
